...
import { type MenuCategoryItem, updateMenuCategoriesAction } from "@/lib/api/menu-category-actions";
...
type WebsiteMenuCategoryManagerProps = {
  allCategories: CategoryOption[];
  initialMenuCategories: MenuCategoryItem[];
};
...
export function WebsiteMenuCategoryManager({ allCategories, initialMenuCategories }: WebsiteMenuCategoryManagerProps) {
  const initialOrderedIds = useMemo(
    () => initialMenuCategories.map((category) => category.id),
        ...
        <div>
          <div className="mt-2 flex items-center gap-2">
            ...
            <div className="group relative">
              <button
                type="button"
                aria-label="Website menu category info"
              ...
              <div className="pointer-events-none absolute left-0 top-full z-20 mt-2 hidden w-80 rounded-2xl border border-slate-200 bg-white p-3 text-sm leading-6 text-slate-600 shadow-xl group-hover:block">
                Organize how categories appear in the website menu. Reorder the list to change the menu order and add or remove categories when needed.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              startSaving(async () => {
                const result = await updateMenuCategoriesAction(orderedIds);
                if (!result.ok) {
        ...
        <div className="space-y-4">
          <Card className="rounded-3xl border border-indigo-100/70 bg-white shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
            <CardHeader className="pb-4">
              <CardTitle>Add To Menu</CardTitle>
            </CardHeader>
