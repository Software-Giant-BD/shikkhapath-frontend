...
import { updateCareerApplicationStatus } from "@/lib/api/career-applications";

type CareerStatusUpdateControlProps = {
  applicationId: number;
  initialStatus: string;
  options: string[];
};

export function CareerStatusUpdateControl({
  applicationId,
  initialStatus,
  options,
}: CareerStatusUpdateControlProps) {
  ...
  const handleUpdate = () => {
    ...
    startTransition(async () => {
      const result = await updateCareerApplicationStatus(applicationId, status);
