...
import { X, Upload, CheckCircle2, Loader2, Heart, Droplets, MapPin, Phone, FileText, Calendar } from "lucide-react";
import { BLOOD_GROUPS } from "@/lib/constants/blood-groups";
import { BANGLADESH_DISTRICTS } from "@/lib/constants/districts";
import { registerDonor } from "@/lib/api/blood-donation";
...
export function DonorRegistrationForm({ onClose, onSuccess }: DonorRegistrationFormProps) {
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white/80 p-8 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight">Register as Donor</h2>
              <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Save Lives with every drop</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-8 p-8 pt-6">
          ...
          <div className="flex flex-col items-center justify-center gap-4">
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 bg-black/40 py-1 text-center">
                <p className="text-[8px] font-black uppercase text-white">Upload</p>
              </div>
            </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full rounded-2xl border-2 border-slate-100 bg-slate-50/50 px-5 py-3.5 text-sm font-bold text-slate-700 outline-none appearance-none transition-all focus:border-red-500 focus:bg-white"
              >
            ...
            {/* NID */}
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">NID Number</label>
              <input
                name="nid_number"
                required
            ...
            {/* Last Donation */}
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">Last Donation (Optional)</label>
              <input
          ...
          <div className="flex items-center justify-between rounded-3xl border-2 border-slate-100 bg-slate-50/30 p-5">
            <div className="space-y-1">
              <p className="text-sm font-black text-slate-800">Current Availability</p>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Are you available for donation now?</p>
            </div>
