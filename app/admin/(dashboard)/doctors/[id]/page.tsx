...
export default async function DoctorDetailsPage(props: {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "active":
        return (
          <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold border border-emerald-200 tracking-wide uppercase">
            Active
          </span>
        );
      case "pending":
        return (
          <span className="px-3 py-1 bg-sky-100 text-sky-700 rounded-full text-xs font-bold border border-sky-200 tracking-wide uppercase">
            Pending
          </span>
        );
      case "rejected":
        return (
          <span className="px-3 py-1 bg-rose-100 text-rose-700 rounded-full text-xs font-bold border border-rose-200 tracking-wide uppercase">
            Rejected
          </span>
        );
      default:
        return (
          <span className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-xs font-bold border border-slate-200 tracking-wide uppercase">
            {status}
          </span>
        );
    }
  ...
  const profileImg = doctor.profile_image_url ?? null;
  const clinicImg = doctor.clinic_image_url ?? null;
      ...
      <div className="flex items-center gap-4 pb-4 border-b border-slate-200/60">
        <Link href="/admin/doctors">
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 text-slate-500 hover:text-slate-800 bg-white shadow-sm border border-slate-200"
          >
              ...
              <div className="mt-8 pt-6 border-t border-slate-100">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      doctor.description || "<p>No description provided.</p>",
                  }}
                />
              </div>
            </CardContent>
