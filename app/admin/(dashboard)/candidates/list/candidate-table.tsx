...
export function CandidateTable({
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Candidate ${status === "approved" ? "approved" : "rejected"} successfully`,
        );
        setCandidates((prev) =>
        ...
        <tbody className="divide-y divide-slate-100 bg-white">
          {candidates.map((candidate) => (
            ...
            >
              <td className="px-6 py-4">
                <div className="flex flex-col">
                  ...
                  <span className="text-slate-500 text-xs">
                    {candidate.education || "No education info"}
                  </span>
                </div>
              </td>
              <td className="px-6 py-4">
              ...
              <td className="px-6 py-4">
                <span
                  className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-bold capitalize ${statusClass(candidate.status)}`}
                >
                  {candidate.status}
                </span>
              </td>
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-2">
                  ...
                  <div className="flex gap-2">
                    {/* Show Approve button ONLY if the candidate is not already approved */}
                    ...
                    {/* Show Reject button ONLY if the candidate is not already rejected */}
