...
export async function getRoutines(
  ...
  const url = query
    ? `/api/ssc-hsc/routines?${query}`
    : `/api/ssc-hsc/routines`;

  try {
    const res = await fetchApi(url, {
      next: { revalidate: 60, tags: ["routines"] },
    });
    ...
    return {
      items: [
        ...
        (r) =>
          (!params?.exam_type || r.exam_type === params.exam_type) &&
          (!params?.year || r.year === params.year),
      ),
...
export async function getResults(
  ...
  try {
    const res = await fetchApi(url, {
      next: { revalidate: 60, tags: ["results"] },
    });
    ...
    if (params?.roll_number)
      mockData = mockData.filter((m) => m.roll_number === params.roll_number);
    if (params?.board_name)
      mockData = mockData.filter((m) => m.board_name === params.board_name);
    if (params?.exam_type)
      mockData = mockData.filter((m) => m.exam_type === params.exam_type);
